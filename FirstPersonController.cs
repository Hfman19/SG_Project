...
public class FirstPersonController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Add inertia to moves
if (curSpeedX == 0 && moveDirection.magnitude > 0f)
{
    moveDirection -= forward * Mathf.Sign(moveDirection.x) * Mathf.Min(moveDirection.magnitude, walkingSpeed) * Time.deltaTime;
}
if (curSpeedY == 0 && moveDirection.magnitude > 0f)
{
    moveDirection -= right * Mathf.Sign(moveDirection.z) * Mathf.Min(moveDirection.magnitude, walkingSpeed) * Time.deltaTime;
}
